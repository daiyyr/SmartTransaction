...
namespace AzureIoTPortal.Web.AzureIoTCore
{
    ...
    public static class EventHub
    {
        ...
        static public async Task<IEnumerable<EventData>> ReceiveMessagesAsync(string connectionString,string DeviceId,DateTime last,long lastQueueNumber = 0,string partition="0")
        {
            ...
            string partitionCur = EventHubPartitionKeyResolver.ResolveToPartition(DeviceId,int.Parse(partition));

            var position = lastQueueNumber > 0 ? EventPosition.FromSequenceNumber(lastQueueNumber) : EventPosition.FromEnqueuedTime(last);

            var eventHubReceiver = eventHubClient.CreateReceiver("$Default", partitionCur, position);
             
            //receive the events from startTime until current time in a single call and process them
            var eventDatas = await eventHubReceiver.ReceiveAsync(5000, TimeSpan.FromSeconds(5));
        ...
        static public async Task SendMessageToDeviceAsync(string connectionString, string DeviceId, Microsoft.Azure.Devices.Message message)
        {
